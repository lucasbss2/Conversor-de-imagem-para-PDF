...
class ImageToPDFConverter:
  ...
  def initialize_ui(self):
    ...
    convert_button = tk.Button(self.root, text = "Convert to PDF", command = self.convert_imagens_to_pdf )
    convert_button.pack(pady=(20,40))
  
  def select_images(self):
    self.image_paths = filedialog.askopenfilenames(title="Select Images",filetypes= [("Image files", "*.png;*.jpeg;*jpg;")])
  ...
  def convert_imagens_to_pdf(self):
    if not self.image_paths:
      return 
    
    current_directory = os.getcwd()
    
    pdf_name = self.output_pdf_name.get() if self.output_pdf_name.get() else "output"
    output_pdf_path = os.path.join(current_directory, pdf_name +".pdf")
